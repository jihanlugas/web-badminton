            ...
            return (
              <div key={key} className='bg-white rounded shadow'>
                <button className='w-full flex justify-between rounded items-center p-4' onClick={() => toggleAccordion(key)}>
                  ...
                  <div className='p-2'>
                    <MdOutlineKeyboardArrowRight className={`rotate-0 duration-500 ${accordion.includes(key) && 'rotate-90'}`} size={'1.5em'} />
                  </div>
                </button>
                <div className={`duration-500 overflow-hidden ${accordion.includes(key) ? 'max-h-60 ' : 'max-h-0 '}`}>
                  <div className='px-4 pb-4'>
                    <div className='flex'>
                      <div className='h-6 w-6 text-gray-500 flex-none flex justify-center items-center mr-2'><FaRegMap className='' size={'1.1rem'} /></div>
                      <div className='flex-grow '>{data.address}</div>
                    </div>
                    <div className='flex'>
                      <div className='h-6 w-6 text-gray-500 flex-none flex justify-center items-center mr-2'>2</div>
                      <div className='flex-grow '>{displayMoney(data.normalGamePrice)}</div>
                    </div>
                    <div className='flex'>
                      <div className='h-6 w-6 text-gray-500 flex-none flex justify-center items-center mr-2'>3</div>
                      <div className='flex-grow '>{displayMoney(data.rubberGamePrice)}</div>
                    </div>
                    <div className='flex'>
                      <div className='h-6 w-6 text-gray-500 flex-none flex justify-center items-center mr-2'><IoBaseballSharp className='' size={'1.1rem'} /></div>
